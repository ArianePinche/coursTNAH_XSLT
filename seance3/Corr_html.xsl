...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="TEI">
        ...
        <body>
            <h1><xsl:apply-templates select="teiHeader"/></h1>
                    
            <xsl:apply-templates select="descendant::body"/>         
            <xsl:element name="div"><xsl:text>Ce </xsl:text>
            <xsl:value-of select="descendant::body/lg/@type"/>
             <xsl:text> comprend </xsl:text> 
            <xsl:value-of select="count(descendant::l)"/><xsl:text> vers.</xsl:text>   
            </xsl:element>
        </body>       
    </html>
    </xsl:template>
    ...
    <xsl:template match="teiHeader">
        <xsl:variable name="titre"><!-- <xsl:variable name="titre" select="descendant::title"/>
        ...
        <xsl:value-of select="concat(descendant::title/text(),' ','écrit par',' ',descendant::author/text())"/>
                                <!-- <xsl:value-of select="concat($titre, ' écrit par ', $auteur)"/> --></xsl:variable>
    ...
    <xsl:template match="l">
        <xsl:element name="li">
            <xsl:attribute name="title"><xsl:number select="." format="1" level="any"/></xsl:attribute>
            <xsl:apply-templates/>
        </xsl:element>
    </xsl:template>
